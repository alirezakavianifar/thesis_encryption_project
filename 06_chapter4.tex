...
یکی از معیارهای اساسی برای ارزیابی مقاومت یک الگوریتم رمزنگاری در برابر حملات جستجوی فراگیر\protect\LTRfootnote{Brute-Force Attacks}، اندازه فضای کلید آن است. فضای کلید نشان‌دهنده تعداد کل حالت‌های ممکن برای کلید رمزنگاری است. برای تضمین امنیت در مقابل ابررایانه‌های امروزی، فضای کلید باید حداقل بزرگ‌تر از $2^{128}$ باشد تا جستجوی همه‌جانبه کلید از نظر محاسباتی غیرممکن گردد.

در الگوریتم پیشنهادی، پارامترهای کلیدی که فضای کلید را تشکیل می‌دهند عبارتند از:
\begin{enumerate}
    \item \textbf{سیستم آشوبی چن:} شامل سه مقدار اولیه ($x_0, y_0, z_0$) و سه پارامتر کنترل ($a, b, c$) که در مجموع ۶ متغیر اعشاری با دقت مضاعف را شامل می‌شود.
    \item \textbf{۹ سیستم آشوبی نمایی ترکیبی (\lr{ECM}):} شامل ۹ مقدار اولیه ($x_{0,1}$ تا $x_{0,9}$) و ۱۸ پارامتر کنترل ($a_1, b_1$ تا $a_9, b_9$) که در مجموع ۲۷ متغیر اعشاری با دقت مضاعف را در بر می‌گیرد.
\end{enumerate}

با فرض استفاده از استاندارد ممیز شناور با دقت مضاعف (دقت ۶۴ بیتی طبق استاندارد \lr{IEEE 754})، دقت هر متغیر اعشاری برابر با $10^{-16}$ است. این دقت معادل این است که برای هر پارامتر اعشاری، حدود $10^{16} \approx 2^{53}$ حالت مختلف وجود دارد. 

بنابراین، کل فضای کلید سیستم با در نظر گرفتن تمام متغیرها برابر است با:
\begin{equation}
\text{Key Space} = (2^{53})^{6 + 27} = 2^{53 \times 33} = 2^{1749}
\end{equation}

حتی در یک سناریوی محافظه‌کارانه که در آن فقط مقادیر اولیه سیستم چن و ۹ سیستم نمایی (مجموعاً ۱۲ متغیر اولیه) به عنوان کلید فعال در نظر گرفته شوند، فضای کلید به صورت زیر خواهد بود:
\begin{equation}
\text{Conservative Key Space} = (2^{53})^{12} = 2^{636}
\end{equation}

از آنجا که فضای کلید محاسباتی الگوریتم پیشنهادی ($2^{1749}$ و در حالت محافظه‌کارانه $2^{636}$) به مراتب بزرگ‌تر از حد آستانه امنیتی $2^{128}$ است، الگوریتم پیشنهادی در برابر هرگونه حمله جستجوی فراگیر مقاومت مطلق نشان می‌دهد.
