...
\\section{رمزنگاری}
رمزنگاری یا رمزگذاری فرآیند تبدیل اطلاعات به‌کمک یک الگوریتم خاص به صورتی غیرقابل‌فهم برای افراد غیرمجاز است. هدف از این کار، ذخیره یا انتقال امن داده‌ها و جلوگیری از دسترسی یا سوء‌استفاده‌ی احتمالی است. در هر سیستم رمزنگاری، کلیدی وجود دارد که تنها در اختیار رمزگذار یا گیرنده‌ی مجاز قرار دارد. این کلید برای معکوس‌سازی عملیات رمزنگاری و بازگردانی داده‌ها به حالت اولیه (رمزگشایی) به‌کار می‌رود.
...
\\begin{itemize}
    ...
    \\item بازرسی و اصلاح الگوریتم‌های عمومی توسط محققان: وقتی الگوریتم و روش پیاده‌سازی عمومی باشد، محققان امنیتی می‌توانند مشکلات و حفره‌های احتمالی آن را پیش از سوء استفاده توسط حمله‌گران شناسایی و رفع کنند؛ بنابراین، سیستم‌های رمزنگاری مبتنی بر الگوریتم‌های عمومی معمولاً قابل اعتمادتر هستند\\cite{ref38}.
\\end{itemize}
...
\\begin{itemize}
    \\item \\textbf{کاهش اضافات و حجم داده:} اضافات زیاد و حجم بالا در داده‌های تصاویر رمزشده، معمولاً باعث آسیب‌پذیری آنها در برابر تحلیلگران و حملات آماری می‌شود\\cite{ref50}. حجم زیاد اطلاعات، امکان استخراج نمونه‌های کافی از متن رمزشده برای تحلیل آماری را فراهم می‌کند، به‌ویژه زمانی که الگوریتم از طول محدود و نسبتاً کوتاه استفاده کند. کاهش حجم داده‌ها و ساده‌تر کردن الگوریتم می‌تواند بار محاسباتی را کاهش دهد و امنیت را بهبود بخشد.
    ...
    \\item \\textbf{غیرقابل پیش‌بینی بودن تصویر رمزشده:} براساس تئوری اطلاعات شانون، یک سیستم رمزنگاری ایمن باید آنتروپی بالایی داشته باشد و تصویر رمزشده نباید هیچ اطلاعاتی درباره تصویر اصلی فاش کند. برای رسیدن به این هدف، تصویر رمزشده باید تا حد امکان تصادفی باشد، هیستوگرام متوازن داشته باشد و هیچ همبستگی آماری بین پیکسل‌های مجاور وجود نداشته باشد. دستیابی به این سطح امنیت نیازمند چندین مرحله جابجایی و جانشانی پیچیده است.
    \\item \\textbf{سرعت رمزنگاری و هزینه محاسباتی:} یک الگوریتم ایمن که بسیار کند اجرا شود یا نیازمند هزینه محاسباتی بالا باشد، برای کاربردهای زمان واقعی ارزش عملی کمی خواهد داشت\\cite{ref53}. بنابراین، سرعت بالا و مصرف منابع کم در طراحی الگوریتم اهمیت ویژه‌ای دارد.
...
مثبت بودن نمای لیاپانوف به معنای وجود رفتار آشوبی در سیستم است. هرچه مقدار نمای لیاپانوف بیشتر باشد، سرعت واگرایی مسیرها بالاتر بوده و در نتیجه سیستم حساسیت بیشتری به شرایط اولیه نشان می‌دهد. در کاربردهای رمزنگاری، بالا بودن این نما به معنای تولید دنباله‌های شبه‌تصادفی با پیچیدگی بیشتر و غیرقابل‌پیش‌بینی بودن بالاتر است که امنیت سیستم را تضمین می‌کند. این مفهوم نشان‌دهنده پایداری یا ناپایداری سیستم در مواجهه با اختلالات کوچک است و به جای استفاده از معادلات پیچیده، بر درک مفهومی واگرایی در فضای فاز تأکید دارد.
...
with open("e:\\projects\\thesis_project\\04_chapter2.tex", "w", encoding="utf-8") as f:
    f.write(content)
