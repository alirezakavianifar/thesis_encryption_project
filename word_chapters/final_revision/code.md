...
# f_ECM(x) = f_outer( exp(mu * f_inner(x)) ) mod 1
...
def encrypt_image(image_bgr, key: dict):
    ...
    # برای هر سیستم یک وضعیت مستقل نگه می‌داریم
    ecm_states = {i: key['x_ecm'] for i in range(1, 10)}
    # warm-up برای هر سیستم
    for i in range(1, 10):
        x = ecm_states[i]
        for _ in range(1000):
    ...
    for c in range(3):
        ...
        ecm_st = dict(ecm_states)   # کپی وضعیت برای هر کانال
        for k in range(N):
            sys_idx = selector[k]
    ...
    return encrypted, perm_inv

def decrypt_image(encrypted_bgr, key: dict, perm_inv):
    ...
    for c in range(3):
        flat = encrypted_bgr[:, :, c].flatten().astype(np.int32)
...
for name, path in IMAGES.items():
    ...
    enc2, _ = encrypt_image(img, key2)
    npcr_val, uaci_val = npcr_uaci(enc, enc2)
    print(f"  NPCR: {npcr_val:.4f}%")
    print(f"  UACI: {uaci_val:.4f}%")

    # ── ذخیره تصاویر ─────────────────────────────────────────────────────
    cv2.imwrite(f'/home/claude/{name}_encrypted.png', enc)
    cv2.imwrite(f'/home/claude/{name}_decrypted.png', dec)
...
"""
تولید تمام نمودارها و تصاویر تحلیلی برای فصل ۴
"""
import pickle
import numpy as np
import cv2
...
with open('/home/claude/results.pkl','rb') as f:
